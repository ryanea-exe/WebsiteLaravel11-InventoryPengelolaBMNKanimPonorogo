...
@section('title', 'Barang Persediaan Keluar')

@section('page-title', 'Barang Persediaan Keluar')
@section('breadcrumb')
    <p class="text-gray-800 cursor-pointer">
        Barang Persediaan Keluar
    </p>
@endsection
    ...
    <div class="bg-white rounded-lg shadow-sm p-4">
        ...
        <div class="flex justify-between mb-4">
            <h1 class="text-xl font-bold text-gray-800">
                <i class="fas fa-file-export mr-3"></i>Daftar Barang Persediaan Keluar
            </h1>
            ...
            <table id="dataTable" class="strip text-gray-700 w-full row-border">
                ...
                @foreach($pengajuans as $pengajuan)
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-2 py-4 text-sm">
                            {{ $pengajuan->tanggal_proses ? \Carbon\Carbon::parse($pengajuan->tanggal_proses)->format('d M Y, H:i') : '-' }}
                        </td>
                        <td class="px-2 py-4 text-sm">
                ...
                if (!data.details || data.details.length === 0) {
                    ...
                    data.details.forEach(item => {
                        tbody.innerHTML += `<tr>
                            ...
                            <td class="px-4 py-2 text-sm text-red-600 font-semibold">${item.jumlah_disetujui} ${item.barang.satuan ?? ''}</td>
                        </tr>`;
                    });
                }
