...
@section('title', 'Riwayat Pajak')

@section('page-title', 'Riwayat Pajak')
@section('breadcrumb')
    ...
    <div class="bg-white rounded-lg shadow-sm p-4">
        ...
        <div class="flex justify-between mb-4">
            <h1 class="text-xl font-bold text-gray-800">
                <i class="fas fa-file-invoice mr-3"></i>Riwayat Pajak Kendaraan
            </h1>
        </div>
        ...
        <div class="overflow-x-auto">
            <table id="dataTable" class="strip text-gray-700 w-full row-border">
                ...
                <tbody>
                    @foreach($riwayatPajak as $i => $r)
                    <tr>
                        <td>{{ $i+1 }}</td>
                        <td>{{ $r->kendaraan->nama_kendaraan ?? '-' }} <b>({{ $r->kendaraan->nomor_polisi ?? '-' }})</b></td>
                        <td>{{ $r->kendaraan->seksi->seksi_singkat }}</td>
                        <td>{{ \Carbon\Carbon::parse($r->tanggal_pajak)->translatedFormat('d F Y') }}</td>
                        <td>{{ $r->nama_pengurus ?? '-' }}</td>
                        <td>{{ $r->keterangan ?? '-' }}</td>
                        <td>
                            <div class="flex space-x-2">
                                <button onclick='openEdit(@json($r->load("kendaraan")))'
                                    title="Edit" class="text-green-600">
                                    <i class="fas fa-edit"></i>
                                </button>
                                <button onclick='openDelete(@json($r->load("kendaraan")))'
                                    title="Hapus" class="text-red-600">
            ...
            <div class="px-6 py-4 overflow-y-auto flex-1">
                <div class="grid grid-cols-1 md:grid-cols-1 gap-4">
                    ...
                    <div>
                        ...
                        <input id="e_tanggal" type="date" name="tanggal_pajak"
    ...
    <div class="modal-box bg-white rounded-2xl w-full max-w-md shadow-lg">
        <div class="p-6 text-center">
            ...
            <h3 class="text-lg font-semibold text-gray-800 mb-2">
                Konfirmasi Hapus Riwayat Pajak
            </h3>
            <p class="text-sm text-gray-600 mb-6">
                Apakah Anda yakin ingin menghapus riwayat pajak
...
<script>
    ...
    function openDetail(data){
        ...
        document.getElementById('d_tanggal').innerText = data.tanggal_pajak;
    ...
    function openEdit(data){
        ...
        form.action = `/pemeliharaan/riwayat_pajak/${data.id}`;

        document.getElementById('editTitle').innerText =
            `Edit Riwayat Pajak - ${data.kendaraan?.nama_kendaraan ?? '-'} (${data.kendaraan?.nomor_polisi ?? '-'})`;
        document.getElementById('e_kendaraan_id').value = data.kendaraan_id;
        document.getElementById('e_tanggal').value = data.tanggal_pajak;
